...
| Platform | Directory |
| --- | --- |
| macOS | `~/Library/Logs/animatem/` |
| Linux | `$XDG_STATE_HOME/animatem/logs/` (default `~/.local/state/...`) |
| Windows | `%LOCALAPPDATA%\animatem\logs\` |

`ANIMATEM_LOG_DIR` overrides it. The path is printed on the first line of every
...
`animatem.YYYY-MM-DD.log`, rotated daily, seven kept, no colour escapes.

Verbosity comes from `ANIMATEM_LOG`, default `info`.
...
```bash
animatem --collect-diagnostics
```

Writes `animatem-diagnostics-<timestamp>.json` and prints its path. One file, so
