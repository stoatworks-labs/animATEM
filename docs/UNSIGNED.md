# Running unsigned animATEM builds
...
animATEM listens on the network, so the first time it starts Windows shows:

> **Windows Defender Firewall has blocked some features of animATEM**
...
domain. AnimATEM needs this to reach the ATEM switcher and expose its Companion control port.

If you deny it (or the prompt appears behind another window and times out), the switcher will show as offline and Companion won't connect
...
**Private**. If animATEM isn't listed, **Allow another app…** → **Browse** to the `.exe`.

Or from an elevated PowerShell:

```powershell
New-NetFirewallRule -DisplayName "animATEM" -Direction Inbound -Program "C:\Path\To\animATEM.exe" -Action Allow -Profile Private,Domain
```

To remove a bad rule and get the prompt back on next launch:

```powershell
Get-NetFirewallRule -DisplayName "*animATEM*" | Remove-NetFirewallRule
```
...
```sh
codesign -dv --verbose=4 "/Applications/animATEM.app"
spctl -a -vv "/Applications/animATEM.app"   # still reports "rejected" — ad-hoc is not notarization
```
